...
export class XeniaGenerator extends BaseGenerator {
  public configure(): void {
    ...
    const configId = isCanary ? 'xenia-canary' : 'xenia';
    ...
    try {
      ...
      const fullscreen = Config.getEmulatorSetting(configId, 'fullscreen', 'false') === 'true';
      
      let gpu = Config.getEmulatorSetting(configId, 'video_driver', 'any');
      if (gpu === 'auto' || !gpu) {
        gpu = 'any';
      } else {
      ...
      const vsync = Config.getEmulatorSetting(configId, 'vsync', 'true') === 'true';
      const licenseMaskSetting = Config.getEmulatorSetting(configId, 'xenia_license_mask', '0');
      ...
      const resolutionSetting = Config.getEmulatorSetting(configId, 'resolution');
      ...
      // Other features
      const allowInvalidFetch = Config.getEmulatorSetting(configId, 'gpu_allow_invalid_fetch_constants');
      ...
      const readbackResolve = Config.getEmulatorSetting(configId, 'd3d12_readback_resolve');
      if (readbackResolve !== undefined) {
        updateTomlSetting('d3d12_readback_resolve', readbackResolve === 'true' || readbackResolve === true);
      }

      const antialiasing = Config.getEmulatorSetting(configId, 'postprocess_antialiasing');
      if (antialiasing) {
        updateTomlSetting('postprocess_antialiasing', antialiasing);
      }

      const allowVrr = Config.getEmulatorSetting(configId, 'xenia_allow_variable_refresh_rate_and_tearing');
      if (allowVrr !== undefined) {
        updateTomlSetting('xenia_allow_variable_refresh_rate_and_tearing', allowVrr === 'true' || allowVrr === true);
      }

      const userLangSetting = Config.getEmulatorSetting(configId, 'xenia_lang');
      if (userLangSetting) {
      ...
      const mountCache = Config.getEmulatorSetting(configId, 'mount_cache');
      if (mountCache !== undefined) {
        updateTomlSetting('mount_cache', mountCache === 'true' || mountCache === true);
      }

      const scribbleHeap = Config.getEmulatorSetting(configId, 'scribble_heap');
      if (scribbleHeap !== undefined) {
        updateTomlSetting('scribble_heap', scribbleHeap === 'true' || scribbleHeap === true);
      }

      const protectZero = Config.getEmulatorSetting(configId, 'protect_zero');
      if (protectZero !== undefined) {
        updateTomlSetting('protect_zero', protectZero === 'true' || protectZero === true);
      }

      const d3dDebug = Config.getEmulatorSetting(configId, 'xenia_d3d12_debug');
      if (d3dDebug !== undefined) {
        updateTomlSetting('d3d12_debug', d3dDebug === 'true' || d3dDebug === true);
      }

      const apu = Config.getEmulatorSetting(configId, 'apu', 'any');
      updateTomlSetting('apu', apu);

      const hidSetting = Config.getEmulatorSetting(configId, 'xenia_hid', 'any');
