...
export class N64RecompGenerator extends BaseGenerator {
      ...
      if (existsSync(schemaPath)) {
        ...
        for (const group of (schema.groups || [])) {
          for (const opt of (group.options || [])) {
            if (opt.realKey) {
              const val = Config.getEmulatorSetting('n64recomplauncher', opt.configKey || opt.id, opt.default || 'auto');
  ...
  public getLaunchCommand(): { executable: string; args: string[] } {
    ...
    const fullscreen = Config.getEmulatorSetting('n64recomplauncher', 'fullscreen', 'true') === 'true';
    if (fullscreen) {
