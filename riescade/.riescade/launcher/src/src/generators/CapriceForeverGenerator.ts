...
export class CapriceForeverGenerator extends BaseGenerator {
  public configure(): void {
    ...
    try {
      const schemaPath = join(getConfigsPath(), 'emulators', 'schemas', 'capriceforever.schema.json');
      if (existsSync(schemaPath)) {
        const schema = JSON.parse(readFileSync(schemaPath, 'utf8'));
        for (const group of (schema.groups || [])) {
          for (const opt of (group.options || [])) {
            if (opt.realKey) {
              const val = Config.getEmulatorSetting('capriceforever', opt.configKey || opt.id, opt.default || 'auto');
