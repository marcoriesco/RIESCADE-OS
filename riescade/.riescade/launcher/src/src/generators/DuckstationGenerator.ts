...
export class DuckstationGenerator extends BaseGenerator {
  public configure(): void {
    ...
    try {
      const fullscreen = (Config.getEmulatorSetting('duckstation', 'fullscreen') ?? Config.getEmulatorSetting('duckstation', 'forcefullscreen') ?? Config.getEmulatorSetting('duckstation', 'duckstation_fullscreen', 'true')) === 'true';
      const vsync = (Config.getEmulatorSetting('duckstation', 'vsync') ?? Config.getEmulatorSetting('duckstation', 'duckstation_vsync', 'true')) === 'true';
      const renderer = Config.getEmulatorSetting('duckstation', 'renderer') ?? Config.getEmulatorSetting('duckstation', 'gfxbackend', 'D3D11');
