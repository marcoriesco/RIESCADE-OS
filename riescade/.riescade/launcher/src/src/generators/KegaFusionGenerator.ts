...
export class KegaFusionGenerator extends BaseGenerator {
  public configure(): void {
    Logger.info(`KegaFusionGenerator: Configuring kega-fusion`);
    ...
    try {
      ...
      Logger.error(`KegaFusionGenerator: Failed to configure Fusion.ini`, err);
    }
  }

  public getLaunchCommand(): { executable: string; args: string[] } {
    const emulatorsDir = getEmulatorsPath();
    const exePath = join(emulatorsDir, 'kega-fusion', 'Fusion.exe');

    if (!existsSync(exePath)) {
      Logger.warn(`KegaFusionGenerator: Executable not found at ${exePath}.`);
    }
