...
export class ForceEngineGenerator extends BaseGenerator {
  public configure(): void {
    Logger.info(`ForceEngineGenerator: Configuring theforceengine`);
    ...
    try {
      ...
      Logger.error(`ForceEngineGenerator: Failed to configure settings.ini`, err);
    }
  }

  public getLaunchCommand(): { executable: string; args: string[] } {
    const emulatorsDir = getEmulatorsPath();
    const exePath = join(emulatorsDir, 'theforceengine', 'TheForceEngine.exe');

    if (!existsSync(exePath)) {
      Logger.warn(`ForceEngineGenerator: Executable not found at ${exePath}.`);
    }
